return function()
	...
	describe("setState suspension", function()
		...
		itFOCUS("should derive state for all setState updates", function()
			local Child = Component:extend("Child")
			local getParentStateCallback
			local stateUpdaterSpy = createSpy(function(state)
				local timesSet = state.timesSet or 0
				return {
					timesSet = timesSet + 1,
				}
			end)
			...
			function Parent.getDerivedStateFromProps(props, state)
				local timesDerived = state.timesDerived or 0
				return {
					timesDerived = timesDerived + 1,
				}
			end
			...
			expect(result).to.be.ok()
			expect(getParentStateCallback().timesSet).to.equal(2)
			expect(getParentStateCallback().timesDerived).to.equal(2)
		end)
	end)
end
